...
We simulate a set of values $n$ called `nset`, at which unbiased estimates `d` are made of values $k_2(n)$, with independent errors with variances `var_w`. We simulate two sets of values `d`: firstly, `d_pTRUE` using the version of $k_2(n)$ satisfying parametric assumptions, and `d_pFALSE` using the version of $k_2(n)$ which does not.
...
We now make estimates of the optimal holdout size using the parametric and semi-parametric (Bayesian emulation) methods. Also see documentation for functions `powersolve_se()` and `ohs_ci` for details and examples of confidence interval construction for such estimates
...
&nbsp;
```{r, echo=FALSE}
actionButton('inc_n1', "Add next suggested point")
...
renderUI({
  plotOutput("unsized1", height = 400, width = 850)
})
```
...
<details><summary>Show detailed code</summary>
```{r,echo=TRUE,fig.width=10,fig.height=5,eval=FALSE}
...
```{r, echo=FALSE}
actionButton('inc_n2', "Add next suggested point")
...
renderUI({
  plotOutput("unsized2", height = 400, width = 850)
})
```
...
Finally, we compare the rates of convergence of OHS estimates. The following plots show medians and interquartile ranges of optimal holdout size estimates from resampled estimates of $k_2(n)$. We compare the random choice of values of $n$ with sequentially systematically-chosen values, as detailed in the previous section.
